...
Daily gold prices: Observations are not independent or identically distributed; strong time dependence and non-stationarity undermine LLN and CLT assumptions.

Course evaluation survey: The sample size is small and likely non-random, so asymptotic results from the LLN and CLT are unreliable.

Pandemic deaths by decade: The data are sparse and heterogeneous across time, with extreme outliers that violate identical distribution assumptions.
...
**Practice:** 
Generate n = 5000 draws from each distribution, compute the cumulative means, and plot the two cumulative-mean series with `ggplot2`, adding a horizontal dashed line at each distribution’s true mean. Use `set.seed()` for reproducibility.
...
cum_norm <- cumsum(x_norm) / seq_len(n)
df_norm <- data.frame(trial = 1:n, cummean = cum_norm, distribution = "Normal (mu=0)")

# Exponential
rate <- 0.5
x_exp <- rexp(n, rate = rate)
cum_exp <- cumsum(x_exp) / seq_len(n)
...
```
When data is resampled 100 times, the sample means distribution deviates substantially from the theoretical distribution. However, as the sample size increases, the sample means ditribution approaches the theoretical distribution. 

Notably, the CLT entails convergence in distribution, not equality. Therefore, for finate samples, the distribution of sample means never becomes truely normal. 
...
p_hist_wi <- ggplot(df, aes(x = writer_confidence)) +
  geom_histogram(aes(y = after_stat(density)),
...
p_hist_pi <- ggplot(df, aes(x = paragraph_informativeness)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 40, fill = "lightgrey", color = "black") +
  geom_density(color = "blue") +
  ylim(0, 0.05) +
...
The distribution of writer confidence is left-skewed. The distribution of paragraph informativeness is right-skewed. Neither variable follows a normal distribution; both display irregular shapes and pronounced peaks near extreme values, indicating deviations from normality.
...
means_pi <- replicate(
  n_samples,
  mean(sample(df$paragraph_informativeness, size = sample_size, replace = TRUE))
)
...
p2 <- ggplot(data.frame(sample_mean = means_pi), aes(x = sample_mean)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 30, fill = "lightgrey", color = "black") +
  labs(
    title = paste0("Paragraph Informativeness"),
    x = "Sample means",
    y = "Density"
  ) +
  geom_density(color = "blue") +
...
# Displaying results next to each other 
library(gridExtra)
...
```
Although the initial distributions of the two variables were not normal, the distribution of sample means for both variables is symmetric and approaches normal distribution.

```

How closely does the distribution of sample means approximate a normal distribution for each of the two variables?

Select one variable, estimate the empirical mean and standard deviation of its sampling distribution, and add the corresponding theoretical normal probability density function to the plot.

```{r CLT-practice 3} 

mean_emp <- mean(means_wc)
sd_emp <- sd(means_wc)

ggplot(data.frame(sample_mean = means_wc), aes(x = sample_mean)) +
  geom_histogram(aes(y = after_stat(density)),
                 bins = 30, fill = "lightgrey", color = "black") +
  labs(
    title = paste0("Writer Confidence"),
...
# Population variable
x <- df$paragraph_informativeness
...
As the sample size increases, the distribution of sample means becomes narrower and more concentrated around the population mean. The variability (spread) of the sample means decreases, leading to more precise and accurate estimates of the true mean. Additionally, the distribution of sample means becomes increasingly symmetric and approaches a normal distribution.
...
Using the `paragraph_informativeness` variable, apply the bootstrap procedure to estimate confidence intervals for the mean at different confidence levels. Set the number of bootstrap resamples to n_samples = 500, and compute 90%, 95%, and 99% percentile bootstrap confidence intervals. 
...
res90  <- bootstrap_ci(x, stat_func = mean, n_samples = 500,   conf = 0.9)
res95 <- bootstrap_ci(x, stat_func = mean, n_samples = 500,  conf = 0.95)
res99 <- bootstrap_ci(x, stat_func = mean, n_samples = 500,  conf = 0.99)

res90$stat     # point estimate (sample mean)
res90$ci       # 90% CI with n_samples = 500
res95$ci       # 95% CI 
res99$ci       # 99% CI 

# Compare widths and endpoints
width <- function(ci) diff(ci)
c(width90 = width(res90$ci), width95 = width(res95$ci), width99 = width(res99$ci))
...
As the confidence level increases from 90% to 95% to 99%, the confidence intervals become wider. The 90% confidence interval is the narrowest, while the 99% confidence interval is the widest. This increase in width reflects greater uncertainty: higher confidence levels require a larger range of values to ensure that the interval captures the true population mean with the specified level of confidence.
...
p_boot_10 <- ggplot(boots10, aes(x = factor(1), y = boot)) +
  ...
  geom_errorbar(data = data.frame(x = factor(1), ymin = ci10_l, ymax = ci10_u),
...
# 2) n_samples = 1000
res1000 <- bootstrap_ci(x = x, stat_func = mean, n_samples = 1000, conf = 0.95)
boots1000 <- data.frame(boot = res1000$boots)
mean1000 <- res1000$stat
ci1000_l <- res1000$ci[1]
ci1000_u <- res1000$ci[2]

p_boot_1000 <- ggplot(boots1000, aes(x = factor(1), y = boot)) +
  geom_boxplot(width = 0.35, outlier.shape = NA, fill = "grey90") +
  geom_errorbar(data = data.frame(x = factor(1), ymin = ci1000_l, ymax = ci1000_u),
                mapping = aes(x = x, ymin = ymin, ymax = ymax),
                inherit.aes = FALSE, color = "red", size = 1.0, width = 0.12) +
  labs(title = "n_samples = 1000", subtitle = paste0("95% CI: [", round(ci1000_l, 3), ", ", round(ci1000_u, 3), "]"),
       x = NULL, y = NULL) +
  ylim(40, 45) +
  theme_minimal()
...
With a small number of resamples (n = 10), the bootstrap distribution is rough and highly variable, leading to a less stable confidence interval that is sensitive to random fluctuations. As the number of bootstrap resamples increases, the bootstrap distribution becomes smoother and more stable, providing a more reliable approximation of the sampling distribution of the mean.
...
```
For a 90% confidence level, the critical Z-value from the Z-table is 1.645. Standard Errors (SE) can be computed as:

standard deviation / sqrt(n) = 10/sqrt(32) = 1.77

Plugging in the values for Z-based CIs gives: 

upper_CI = mean + 1.645 * SE = 52 + 1.645 * 1.77 = 54.91
lower_CI = mean - 1.645 * SE = 52 - 1.645 * 1.77 = 49.09

90% confidence interval for the population mean: (49.09, 54.91)
...
Importantly, we can use the results of the results of the CLT to calculate CIs for population mean even when the sample itself is not normally distributed. 

Using `paragraph_informativeness`, calculate 90%, 95%, and 99% Z-based confidence intervals for the population mean.

**Hint:** The formula for Z-based CIs: 

$$
\left(
\bar{x} - z_{1-\alpha/2}\,*{SE},
\;
\bar{x} + z_{1-\alpha/2}\,*{SE}
\right),
$$
...
# Point estimate and standard error for the population mean
x_bar <- mean(df$paragraph_informativeness)     # sample mean
se_hat <- sd(df$paragraph_informativeness) / sqrt(length(df$paragraph_informativeness))  # standard error of the mean
...
ci_z <- lapply(alphas, function(alpha) {
  z <- qnorm(1 - alpha / 2)
  c(
    lower = x_bar - z * se_hat,
    upper = x_bar + z * se_hat
  )
})
...
mu_true <- mean(df$writer_confidence)
...
for (i in 1:5000) {
  ...
  z_lower <- xbar - 1.96 * s / sqrt(n)
  z_upper <- xbar + 1.96 * s / sqrt(n)
...
**Extra:** The same probability-theory approach can be applied to categorical (binary) data. Instead of estimating a population mean, we now estimate a population proportion.
...
The chi-squared distribution is defined only for non-negative values, is asymmetric (right-skewed), and its shape depends on the degrees of freedom. In contrast, the normal distribution is symmetric, bell-shaped, and defined over the entire real line.

As the degrees of freedom increase, the chi-squared distribution becomes less skewed and more symmetric. Its mean and variance increase, and the distribution gradually approaches a normal distribution in shape, centered around its mean. 
...
n <- 30
true_sigma <- 10
x <- rnorm(n, mean = 50, sd = true_sigma)

s2 <- var(x)
deg_freedom <- n - 1
alpha <- 0.05

chi_lower <- qchisq(1 - alpha/2, df = deg_freedom)  # upper critical value
chi_upper <- qchisq(alpha/2, df = deg_freedom)      # lower critical value

# CI for variance
ci_var <- c(
  lower = deg_freedom * s2 / chi_lower,
  upper = deg_freedom * s2 / chi_upper
)
